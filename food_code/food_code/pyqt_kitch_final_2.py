    ...
    def send_message(self):
        ...
        if self.node is not None:
            for table_num in self.selected_tables:
                ...
                data_with_zero = self.selected_tables[:]
                if data_with_zero[-1] != 0:
                    data_with_zero.append(0)

                msg.data = data_with_zero  # 메시지 데이터 설정
...
class KitchenSubscriber(Node):
    def __init__(self, gui):
        ...
        qos_profile_2 = QoSProfile(
            depth=10,
            reliability=QoSReliabilityPolicy.RELIABLE  # 올바른 설정
        )
        ...
        qos_profile_1 = QoSProfile(
            ...
            #reliability=QoSReliabilityPolicy.RELIABLE  # 추가 가능
        )

        self.publisher = self.create_publisher(Int32MultiArray, 'table_topic', qos_profile_1)
